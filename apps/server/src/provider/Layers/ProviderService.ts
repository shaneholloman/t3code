...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  const startSession: ProviderServiceShape["startSession"] = Effect.fn(
    "startSession",
    ...
    return yield* Effect.gen(function* () {
      ...
      const legacyProviderSettings =
        resolvedProvider in settings.providers
          ? settings.providers[
              resolvedProvider as keyof typeof settings.providers
            ]
          : undefined;
      if (
        legacyProviderSettings !== undefined &&
        !legacyProviderSettings.enabled
      ) {
        return yield* toValidationError(
          "ProviderService.startSession",
  ...
  const interruptTurn: ProviderServiceShape["interruptTurn"] = Effect.fn(
    "interruptTurn",
  )(function* (rawInput) {
  ...
  const respondToRequest: ProviderServiceShape["respondToRequest"] = Effect.fn(
    "respondToRequest",
  )(function* (rawInput) {
    const input = yield* decodeInputOrValidationError({
  ...
  const listSessions: ProviderServiceShape["listSessions"] = Effect.fn(
    "listSessions",
  )(function* () {
    const currentAdapters = yield* getAdapterEntries;
  ...
  const getInstanceInfo: ProviderServiceShape["getInstanceInfo"] = (
    instanceId,
